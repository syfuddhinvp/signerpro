  ...
  return (
    <div style={{ padding: '26px 28px', fontFamily: SF_FONT, display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '13px' }}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ background: '#fff', border: '1px solid #e3e7ee', borderRadius: '13px', padding: '17px', display: 'flex', flexDirection: 'column', gap: '11px' }}>
            <div style={bar('60%', '10px')} />
            <div style={bar('42%', '22px')} />
          </div>
        ))}
      </div>
      <div style={{ background: '#fff', border: '1px solid #e3e7ee', borderRadius: '13px', padding: '17px', display: 'flex', flexDirection: 'column', gap: '13px' }}>
