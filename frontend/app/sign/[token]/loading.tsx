...
export default function SignLoading() {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, fontFamily: SF_FONT, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: '0 0 auto', background: 'hsl(var(--color-bg-surface))', borderBottom: '1px solid hsl(var(--color-border-subtle))', padding: '11px 18px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
          <div style={bar('220px', '12px')} />
          <div style={{ height: '6px', borderRadius: '99px', background: 'hsl(var(--color-bg-muted))', maxWidth: '420px' }} />
        </div>
        <div style={bar('130px', '32px')} />
      </div>
      <div style={{ flex: 1, minHeight: 0, padding: '24px', display: 'flex', justifyContent: 'center' }}>
        <div style={{ width: '816px', maxWidth: '100%', height: '520px', background: 'hsl(var(--color-bg-surface))', borderRadius: '3px', boxShadow: '0 24px 60px -24px rgba(15,23,42,.35), 0 0 0 1px hsl(var(--color-border-default))', padding: '64px 72px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
