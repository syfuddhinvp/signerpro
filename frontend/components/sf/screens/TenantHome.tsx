...
const templateCard: CSSProperties = { border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '13px', padding: '10px', display: 'flex', flexDirection: 'column', gap: '9px', background: 'hsl(var(--color-bg-surface))' };
...
const templateTitle: CSSProperties = { fontSize: '.78125rem', fontWeight: 600, color: 'hsl(var(--color-fg-default))', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
/** The preview well the mini sheets sit in — a page's 4:3 crop, not a full A4. */
const previewSheet: CSSProperties = { position: 'relative', display: 'block', height: '86px', borderRadius: '9px', background: 'hsl(var(--color-bg-muted))', border: '1px solid hsl(var(--color-border-hairline))', overflow: 'hidden' };
/** The offset sheet behind, shown only for a multi-page template. */
const sheetBack: CSSProperties = { position: 'absolute', width: '58%', height: '78%', background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '4px' };
...
export default function TenantHome({
  ...
  const orgStats = stats.map(x => ({
    ...
    metaStyle: { fontSize: '.71875rem', fontWeight: 600, fontFamily: 'var(--font-sans)', color: x.good ? 'hsl(var(--color-fg-success))' : 'hsl(var(--color-fg-warning))' } as CSSProperties,
    bar: { width: x.pct + '%', height: '100%', borderRadius: '99px', background: x.good ? 'hsl(var(--color-highlight-solid))' : 'hsl(var(--color-bg-warning-solid))' } as CSSProperties,
  }));
  ...
  const orgChart = series.map((v, i) => ({
    ...
    bar: { width: '100%', height: Math.round(v / peak * 100) + '%', borderRadius: '5px 5px 2px 2px', background: i === series.length - 1 ? A : 'hsl(var(--color-accent-muted))' } as CSSProperties,
  }));
  ...
  const orgAttention = attention.map(a => ({
    ...
    rowStyle: { display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 10px', border: '1px solid hsl(var(--color-border-hairline))', borderRadius: '11px', background: 'hsl(var(--color-bg-subtle))', cursor: 'pointer', width: '100%' } as CSSProperties,
  ...
  const orgTeam = team.map(t => ({
    ...
    chip: { width: '28px', height: '28px', borderRadius: '99px', background: 'hsl(var(--color-border-subtle))', color: 'hsl(var(--color-fg-subtle))', display: 'grid', placeItems: 'center', fontSize: '.6875rem', fontWeight: 700, flex: '0 0 28px' } as CSSProperties,
  }));

  const orgBannerStyle: CSSProperties = { background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '14px', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap' };
  const orgAvatar: CSSProperties = { width: '38px', height: '38px', borderRadius: '11px', background: 'hsl(var(--color-bg-panel-dark))', color: 'hsl(var(--color-fg-on-solid))', display: 'grid', placeItems: 'center', fontSize: '.8125rem', fontWeight: 700, flex: '0 0 38px' };
  ...
  return (
    <section data-screen-label="Tenant overview" style={{ padding: '22px 22px 40px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={orgBannerStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
            ...
            <span style={{ fontSize: '.71875rem', color: 'hsl(var(--color-fg-muted))', fontFamily: 'var(--font-sans)' }}>{banner.meta}</span>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: '12px' }}>
        {orgStats.map(s => (
          <div key={s.label} style={{ background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '14px', padding: '15px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <span style={{ fontSize: '.65625rem', letterSpacing: '.06em', color: 'hsl(var(--color-fg-muted))', fontFamily: 'var(--font-sans)' }}>{s.label}</span>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
              <span style={{ fontSize: '1.5625rem', fontWeight: 700, letterSpacing: '-.9px' }}>{s.value}</span>
              <span style={s.metaStyle}>{s.meta}</span>
            </div>
            <div style={{ height: '4px', borderRadius: '99px', background: 'hsl(var(--color-bg-muted))', overflow: 'hidden' }}><div style={s.bar}></div></div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
          ...
          <Link href={folderHref('templates')} style={{ fontSize: '.71875rem', color: 'hsl(var(--color-accent-solid))', fontWeight: 600, textDecoration: 'none' }}>
              ...
              <div key={t.templateId} style={templateCard}>
                ...
                <button type="button" onClick={() => useTemplate(t)} title={'Start an envelope from ' + t.title} style={templateOpen}>
                    ...
                    <span style={sheetFace}>
                      ...
                      <span style={{ display: 'flex', gap: '4px', marginTop: 'auto' }}>
                        {Array.from({ length: Math.min(Math.max(t.fields, 1), 3) }).map((_, i) => (
                          <span key={i} style={{ ...fieldChip, background: i === 0 ? A : 'hsl(var(--color-accent-muted))' }} />
                        ))}
                      </span>
                    </span>
                  </span>
                  <span style={{ display: 'flex', flexDirection: 'column', gap: '3px', textAlign: 'left', minWidth: 0, width: '100%' }}>
                    ...
                    <span style={{ fontSize: '.65625rem', color: 'hsl(var(--color-fg-muted))', fontFamily: 'var(--font-sans)' }}>{t.meta}</span>
                  </span>
                </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: '16px', alignItems: 'start' }}>
        <div style={{ background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: '.6875rem', color: 'hsl(var(--color-fg-muted))', fontFamily: 'var(--font-sans)' }}>peak {peak} / wk</span>
          </div>
          ...
          {orgAttention.length === 0 ? (
            ...
            <button key={a.label} type="button" onClick={a.onClick} style={a.rowStyle}>
              ...
              <span style={{ display: 'flex', flexDirection: 'column', gap: '2px', textAlign: 'left', minWidth: 0, flex: 1 }}>
                <span style={{ fontSize: '.78125rem', fontWeight: 600, color: 'hsl(var(--color-fg-default))' }}>{a.label}</span>
                <span style={{ fontSize: '.6875rem', color: 'hsl(var(--color-fg-muted))' }}>{a.meta}</span>
              </span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: '16px', alignItems: 'start' }}>
        <div style={{ background: 'hsl(var(--color-bg-surface))', border: '1px solid hsl(var(--color-border-subtle))', borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '11px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '9px' }}>
            ...
            <span style={{ fontSize: '.75rem', color: 'hsl(var(--color-fg-muted))' }}>{nextInvoiceMeta}</span>
          </div>
          {spend.lines.length === 0 ? (
            <span style={emptyNote}>No invoiced usage in this period yet.</span>
          ) : spend.lines.map(l => (
            <div key={l.k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.75rem', padding: '6px 0', borderTop: '1px solid hsl(var(--color-border-faint))' }}>
              <span style={{ color: 'hsl(var(--color-fg-muted))' }}>{l.k}</span><span style={{ fontFamily: 'var(--font-sans)', fontWeight: 500 }}>{l.v}</span>
            </div>
          ))}
          ...
          {orgTeam.length === 0 ? (
            ...
            <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: '11px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', flex: 1, minWidth: 0 }}>
                ...
                <span style={{ fontSize: '.6875rem', color: 'hsl(var(--color-fg-muted))', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.meta}</span>
              </div>
