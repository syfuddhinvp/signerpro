...
def test_non_finite_values_are_not_numbers(literal: str) -> None:
    ...
    ``NaN`` was the dangerous one: it did not raise, so it flowed through and
    stamped the literal string "NaN" into an executed contract as an amount.
    ``Infinity`` instead blew up ``quantize`` and 500'd the signing endpoint.
    """
    assert currency_service.to_decimal(literal) is None
    assert currency_service.format_currency(literal) is None
...
def test_unquantizable_magnitudes_are_rejected_too() -> None:
    ...
    assert currency_service.to_decimal("1e999") is not None  # genuinely "finite"
    assert currency_service.format_currency("1e999") is None
...
def test_format_is_inside_the_error_guard() -> None:
    ...
    huge = "9" * 400
    result = currency_service.format_currency(huge)
