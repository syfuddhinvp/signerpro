...
| **W7** | Access-token refresh race | 15-min tokens, 30s skew, backend rotation revokes on first use (`auth_service.py:243`), so the loser of a concurrent refresh gets a 401 and is logged out | **Escalate.** Not the mechanical fix it looked like: the refresh token is stored only as a hash, so the successor's token cannot be re-handed to the loser. Every workable grace window therefore re-issues a session against a token already spent — a real security tradeoff (it also widens replay of a stolen refresh token by the width of the window). Recommend a tight window (≤10s) plus reuse-detection that revokes the whole chain outside it, but this is a session-security decision, not an implementation detail |
